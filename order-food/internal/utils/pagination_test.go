...
func TestBuildPaginationLinks_FirstPage(t *testing.T) {
	...
	// Check self link
	assert.Equal(t, "/api/v1/products?page=1&perPage=10", links[0].Href)
	assert.Equal(t, "self", links[0].Rel)

	// Check next link
	assert.Equal(t, "/api/v1/products?page=2&perPage=10", links[1].Href)
	assert.Equal(t, "next", links[1].Rel)

	// Check last link
	assert.Equal(t, "/api/v1/products?page=5&perPage=10", links[2].Href)
...
func TestBuildPaginationLinks_MiddlePage(t *testing.T) {
	...
	// Check self link
	assert.Equal(t, "/api/v1/products?page=3&perPage=10", links[0].Href)
	assert.Equal(t, "self", links[0].Rel)

	// Check first link
	assert.Equal(t, "/api/v1/products?page=1&perPage=10", links[1].Href)
	assert.Equal(t, "first", links[1].Rel)

	// Check prev link
	assert.Equal(t, "/api/v1/products?page=2&perPage=10", links[2].Href)
	assert.Equal(t, "prev", links[2].Rel)

	// Check next link
	assert.Equal(t, "/api/v1/products?page=4&perPage=10", links[3].Href)
	assert.Equal(t, "next", links[3].Rel)

	// Check last link
	assert.Equal(t, "/api/v1/products?page=5&perPage=10", links[4].Href)
	assert.Equal(t, "last", links[4].Rel)
}

func TestBuildPaginationLinks_LastPage(t *testing.T) {
	links := BuildPaginationLinks(5, 5, "/api/v1/products", 10)

	// Should have self, first, and prev links
	assert.Len(t, links, 3)

	// Check self link
	assert.Equal(t, "/api/v1/products?page=5&perPage=10", links[0].Href)
	assert.Equal(t, "self", links[0].Rel)
	...
	// Check prev link
	assert.Equal(t, "/api/v1/products?page=4&perPage=10", links[2].Href)
	assert.Equal(t, "prev", links[2].Rel)
}
...
func TestBuildPaginationLinks_DifferentBasePath(t *testing.T) {
	links := BuildPaginationLinks(1, 3, "/api/v1/orders", 20)

	// Check that base path is used correctly
	assert.Equal(t, "/api/v1/orders?page=1&perPage=20", links[0].Href)
	assert.Equal(t, "/api/v1/orders?page=2&perPage=20", links[1].Href)
	assert.Equal(t, "/api/v1/orders?page=3&perPage=20", links[2].Href)
}

func TestBuildPaginationLinks_AllLinksHaveMethod(t *testing.T) {
	links := BuildPaginationLinks(2, 3, "/api/v1/products", 10)
